...
export default function DatasetPage() {
  ...
  const handleArchive = async (datasetId: string) => {
    if (!confirm("Are you sure you want to archive this dataset? Historical dashboards and reports referring to it will remain preserved.")) return;
    try {
  ...
  const fetchProfile = useCallback(async (datasetId: string, force: boolean = false) => {
  ...
  const handleDelete = async (datasetId: string, name: string) => {
    if (!confirm(`Are you sure you want to remove '${name}' from your workspace catalog?`)) {
      return;
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.75rem" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "1.2rem", marginTop: "0.3rem", fontSize: "0.8125rem", color: "var(--text-secondary)", flexWrap: "wrap" }}>
                <span>Size: <strong style={{ color: "var(--text-primary)" }}>{formatBytes(activeDataset.file_size_bytes)}</strong></span>
                <span>DuckDB View: <code style={{ color: "var(--accent-indigo)", background: "rgba(99, 102, 241, 0.1)", padding: "0.1rem 0.35rem", borderRadius: "4px" }}>{activeDataset.duckdb_table_name}</code></span>
                {activeDataset.sheet_name && (
                  <span>Worksheet: <strong style={{ color: "var(--text-primary)" }}>{activeDataset.sheet_name}</strong></span>
                )}
          ...
          <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <Link href="/data-quality" className="btn btn-secondary btn-sm" style={{ gap: "0.4rem" }}>
              <DataQualityIcon size={14} />
              <span>Data Quality</span>
            </Link>
            <Link href="/sql" className="btn btn-primary btn-sm" style={{ gap: "0.4rem" }}>
              <SQLIcon size={14} />
      ...
      {/* ── PHASE 4: DATASET INTELLIGENCE & PROFILING VIEW ── */}
      {activeDataset && (
        <>
          ...
          {isProfileLoading && (
            ...
            >
              <div style={{ padding: "2.5rem 1rem", textAlign: "center" }}>
                ...
                />
                <p style={{ margin: 0, fontSize: "0.9375rem", fontWeight: 500, color: "var(--text-primary)" }}>
                  Analyzing schema, calculating distributions, and detecting semantic types...
                </p>
                <p style={{ margin: "0.35rem 0 0", fontSize: "0.8125rem", color: "var(--text-muted)" }}>
                  Stage: Computing quantiles, IQR, variance, and target candidates via DuckDB engine.
                </p>
              </div>
            </SectionCard>
          )}
          ...
          {profileError && !isProfileLoading && (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                ...
                <div>
                  <h4 style={{ margin: 0, fontSize: "0.9375rem", fontWeight: 600, color: "var(--accent-rose)" }}>
      ...
      >
        ...
        {isDatasetsLoading ? (
          ...
          <div style={{ padding: "3rem 1rem", textAlign: "center" }}>
            ...
            <p style={{ margin: 0, fontSize: "0.9375rem", color: "var(--text-secondary)", fontWeight: 500 }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table className="table" style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  <th style={{ textAlign: "left", padding: "0.75rem 1rem" }}>Dataset Name</th>
                  <th style={{ textAlign: "left", padding: "0.75rem 1rem" }}>Format</th>
                  <th style={{ textAlign: "left", padding: "0.75rem 1rem" }}>File Size</th>
                  <th style={{ textAlign: "left", padding: "0.75rem 1rem" }}>DuckDB Identifier</th>
                  <th style={{ textAlign: "left", padding: "0.75rem 1rem" }}>Status</th>
                  <th style={{ textAlign: "left", padding: "0.75rem 1rem" }}>Uploaded</th>
                ...
                {datasets.map((ds: DatasetResponse) => {
                  ...
                  return (
                    <tr
                      key={ds.id}
                      style={{
                        backgroundColor: isActive ? "rgba(16, 185, 129, 0.04)" : "transparent",
                    ...
                    >
                      <td style={{ padding: "0.85rem 1rem" }}>
                        ...
                        <div style={{ fontSize: "0.6875rem", color: "var(--text-muted)", marginTop: "0.15rem", fontFamily: "var(--font-mono)" }}>
                          {ds.id}
                        </div>
                      </td>
                      <td style={{ padding: "0.85rem 1rem" }}>
                      ...
                      <td style={{ padding: "0.85rem 1rem", fontFamily: "var(--font-mono)", fontSize: "0.8125rem", color: "var(--text-secondary)" }}>
                      ...
                      <td style={{ padding: "0.85rem 1rem", fontSize: "0.8125rem", color: "var(--text-muted)" }}>
